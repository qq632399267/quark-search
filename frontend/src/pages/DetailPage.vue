        ...
        <div class="detail-layout">
          ...
          <div class="detail-poster">
            <div class="poster-sticky">
              ...
              <div class="poster-actions">
                <n-button type="primary" size="large" block @click="saveToCloud">
                  🚀 转存到网盘
                </n-button>
                <n-button
          ...
          <div class="detail-info">
            ...
            <div class="info-stats">
              ...
              <div class="stat-item">
                <span class="stat-value">{{ resource.source }}</span>
                <span class="stat-label">网盘来源</span>
              </div>
            </div>
            ...
            <div class="info-section">
              ...
              <div class="link-box">
                ...
                <div class="link-actions">
                  ...
                  <n-button size="small" type="primary" @click="saveToCloud">转存</n-button>
                </div>
              </div>
              <div class="link-tip">
                ⚠️ 链接可能有时效，请尽快转存到你的网盘。本站仅提供搜索索引，不存储任何文件。
              </div>
            </div>
          </div>
        </div>
...
import { toggleFavorite, isFavorite } from '../api/favorites.js'
...
async function loadDetail() {
  ...
  try {
    ...
    if (res.data) isFav.value = isFavorite(res.data.id)
...
function toggleFav() {
  ...
  isFav.value = toggleFavorite(resource.value.id)
  message.success(isFav.value ? '✅ 已收藏' : '已取消收藏')
}

function copyLink() {
  if (resource.value?.link) {
    navigator.clipboard.writeText(resource.value.link).then(() => {
      message.success('链接已复制，快去网盘转存吧！')
...
.detail-poster { flex-shrink: 0; width: 320px; }
.poster-sticky { position: sticky; top: 76px; }
.poster-img { width: 100%; border-radius: 14px; box-shadow: 0 4px 20px rgba(0,0,0,0.12); display: block; }
.poster-actions { margin-top: 16px; }

.detail-info { flex: 1; min-width: 0; }
.info-header { margin-bottom: 20px; }
.info-title { font-size: 26px; font-weight: 700; color: #1a1a1a; line-height: 1.4; margin-bottom: 12px; }
.info-tags { display: flex; flex-wrap: wrap; gap: 8px; }
.tag { padding: 4px 12px; border-radius: 6px; font-size: 13px; font-weight: 500; }
...
.info-section-title { font-size: 15px; font-weight: 600; color: #1a1a1a; margin-bottom: 8px; }
.info-section-text { font-size: 14px; color: #555; line-height: 1.7; }

.link-box {
  display: flex; align-items: center; justify-content: space-between; gap: 12px;
  padding: 14px 16px; background: #f8f9fa; border-radius: 10px; border: 1px solid #eee;
}
.link-url { flex: 1; font-size: 14px; color: #333; word-break: break-all; font-family: monospace; }
.link-actions { display: flex; gap: 6px; flex-shrink: 0; }
.link-tip { margin-top: 10px; font-size: 12px; color: #e67e22; background: #fff8e6; padding: 8px 14px; border-radius: 8px; }

@media (max-width: 768px) {
  .detail-layout { flex-direction: column; }
  .detail-poster { width: 100%; max-width: 280px; margin: 0 auto; }
  .poster-sticky { position: static; }
  .info-title { font-size: 22px; }
  .info-stats { flex-wrap: wrap; gap: 16px; }
